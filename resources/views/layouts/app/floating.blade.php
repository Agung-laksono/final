...
{{-- Logout --}}
            ...
            <div class="flex items-center gap-4 transition-all duration-300 transform origin-left" x-bind:class="isNotifOpen ? 'opacity-0 scale-75 pointer-events-none' : 'opacity-100 scale-100'">
                <button @click="open = !open; if(!open) setTimeout(() => activeMenu = null, 300); else activeMenu = null;" 
                        :class="isIdle && !open ? 'w-[30px] h-[30px] lg:w-16 lg:h-16 opacity-60' : 'w-12 h-12 lg:w-16 lg:h-16 opacity-100'"
                ...
                <div class="flex items-center gap-3 transition-all duration-300 transform origin-left opacity-60 hover:opacity-100" 
                     x-bind:class="open ? 'opacity-60 translate-x-0 scale-100' : 'opacity-0 -translate-x-8 scale-75 pointer-events-none'">
                    {{-- Chat Shortcut --}}
                    <a href="{{ route('chat.index') }}" wire:navigate
