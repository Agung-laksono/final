...
{{-- Toggle View (Mobile: Single Button Toggle) --}}
<div class="md:hidden">
    @if ($viewMode === 'grid')
        <flux:button wire:click="setViewMode('table')" icon="table-cells" class="px-2" title="Tampilan Tabel" />
    @else
        <flux:button wire:click="setViewMode('grid')" icon="squares-2x2" class="px-2" title="Tampilan Grid" />
    @endif
</div>
<div class="hidden md:flex bg-zinc-100 dark:bg-zinc-800 p-1 rounded-lg">
