...
<div x-data="{ collapsed: $persist({{ $defaultCollapsed ? 'true' : 'false' }}).as('kanban-col-{{ $componentId }}-{{ $statusKey }}-user-{{ auth()->id() }}') }"
     ...
     class="flex-shrink-0 rounded-xl transition-all duration-300 snap-center bg-zinc-50/50 dark:bg-zinc-800/40 border border-zinc-200/60 dark:border-zinc-700/50"
