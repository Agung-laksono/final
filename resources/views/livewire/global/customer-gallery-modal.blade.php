...
<div>
    <flux:modal name="customer-gallery-modal" class="md:w-[800px] max-w-4xl">
        <div class="space-y-6">
            ...
            <div wire:loading.remove wire:target="searchQuery" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 max-h-[55vh] overflow-y-auto p-1 custom-scrollbar">
                @forelse($customers as $customer)
                    <div wire:click="$dispatch('customer-selected', { customerId: {{ $customer->id }} })" 
                         x-on:click="$flux.modal('customer-gallery-modal').close()"
