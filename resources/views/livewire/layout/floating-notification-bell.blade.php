...
<div x-data="{ openNotifs: false, isLoading: false }" class="relative z-50" @click.outside="openNotifs = false" x-init="$watch('openNotifs', value => $dispatch('notifs-toggled', value))">
    ...
    {{-- Teleport BOTH Backdrop and Floating Notification List to body --}}
    <template x-teleport="body">
        <div x-show="openNotifs" style="display: none;" class="relative">
            
            {{-- 1. Full-Screen Backdrop Blur (Covers and blurs ALL elements behind) --}}
            <div x-show="openNotifs" 
                 ...
                 class="fixed inset-0 bg-white/60 dark:bg-zinc-950/70 backdrop-blur-md z-[9998]"
                 @click="openNotifs = false">
            </div>

            {{-- 2. Floating Notification List (Elevated ABOVE Backdrop with z-[9999] so cards stay sharp) --}}
            <div x-show="openNotifs" 
                 ...
                 class="fixed bottom-16 sm:bottom-20 left-3 sm:left-6 z-[9999] flex flex-col gap-2 items-start w-max">
                 
                 {{-- Action Header / Utility --}}
                 <div class="flex items-center justify-between gap-3 w-full opacity-90 mb-1 mt-2">
                     <div class="flex items-center gap-2">
                         <div class="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></div>
                         <span class="text-[11px] font-extrabold text-zinc-800 dark:text-zinc-200 uppercase tracking-widest">
                             Daftar Notifikasi Terbaru
                         </span>
                     </div>
                     <button @click="openNotifs = false" class="p-1 rounded-full bg-zinc-200/80 dark:bg-zinc-800/80 text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-colors" title="Tutup">
                         <flux:icon.x-mark class="w-3.5 h-3.5" />
                     </button>
                 </div>

                 {{-- Scrollable Container (Max height constrained so it leaves top spacing) --}}
                 <div class="flex flex-col gap-1.5 w-[calc(100vw-2rem)] sm:w-[400px] max-h-[calc(100vh-14rem)] sm:max-h-[360px] overflow-y-auto px-3 -ml-3 custom-scrollbar pb-4 pt-1">
                     @forelse ($notifications as $notification)
                         <div wire:key="notif-{{ $notification->id }}" 
                             ...
                             <div class="flex flex-col bg-white/95 dark:bg-zinc-800/95 backdrop-blur-sm border border-zinc-200 dark:border-zinc-700 px-3 py-2 rounded-xl shadow-xl group-hover:scale-[1.02] transition-transform origin-left flex-1 min-w-0 {{ $notification->read_at ? 'opacity-75 grayscale-[20%]' : 'shadow-indigo-500/10 border-indigo-500/30' }}">
                     ...
                     <button wire:click="loadMore" wire:loading.attr="disabled" class="flex items-center justify-center gap-2 w-full mt-2 py-2 text-[10px] font-bold text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-200 uppercase tracking-widest bg-zinc-100 dark:bg-zinc-800/80 hover:bg-zinc-200 dark:hover:bg-zinc-700/80 rounded-xl transition-colors outline-none focus:ring-2 focus:ring-indigo-500/50">
