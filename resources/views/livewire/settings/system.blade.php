...
new class extends Component {
    ...
    public function createBackup() {
        ...
        if ($zip->open($backupPath, \ZipArchive::CREATE | \ZipArchive::OVERWRITE) === TRUE) {
            ...
            if ($this->googleDriveEnabled && Storage::disk('local')->exists('google-drive-credentials.json')) {
                try {
                    \Illuminate\Support\Facades\Config::set('filesystems.disks.google.folder', $this->googleDriveFolderId);
                    app('filesystem')->forgetDisk('google');
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <div class="relative flex items-center"
                         x-data="{ isUploading: false, progress: 0, fileSize: null }"
                         x-on:livewire-upload-start="isUploading = true; progress = 0"
                         x-on:livewire-upload-finish="isUploading = false"
                         x-on:livewire-upload-error="isUploading = false; alert('Gagal mengunggah file. Ukuran file (' + fileSize + ' MB) mungkin melebihi batas maksimal (upload_max_filesize) di server Anda, atau batas post_max_size PHP.')"
                        ...
                        <label class="cursor-pointer bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 text-sm font-medium h-8 px-3 rounded-lg border border-zinc-200 dark:border-zinc-700 transition-colors flex items-center gap-2 whitespace-nowrap shadow-sm">
                            ...
                            <input type="file" wire:model="uploadedBackupFile" class="hidden" accept=".zip,.sqlite" 
                                x-on:change="fileSize = $event.target.files.length ? ($event.target.files[0].size / 1024 / 1024).toFixed(1) : null" />
                        </label>
                        
                        <!-- Progress Bar (Alpine) -->
                        <div x-show="isUploading" class="absolute right-0 -bottom-8 w-[150px]" style="display: none;">
                            ...
                            <div class="text-[10px] text-zinc-500 text-right mt-1 font-medium" 
                                x-text="progress === 100 ? 'Memproses... mohon tunggu' : `Mengunggah... ${progress}% ` + (fileSize ? `(${fileSize} MB)` : '')">
                            </div>
                        </div>
