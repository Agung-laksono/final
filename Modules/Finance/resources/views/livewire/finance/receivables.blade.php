    ...
    public function with()
    {
        ...
        foreach ($grouped as $key => $collection) {
            ...
            if ($key === 'partial') {
                ...
                $totals[$key] = 'Rp ' . number_format($sumPaid, 0, ',', '.') . ' / Rp ' . number_format($sumTotal, 0, ',', '.') . ' <span class="text-red-500 font-bold ml-1">(-Rp ' . number_format(max(0, $sisa), 0, ',', '.') . ')</span>';
    ...
    @foreach($columns as $colKey => $column)
        ...
        >
            <x-slot name="headerSubtitle">
                @if(($totals[$colKey] ?? '') !== 'Rp 0' && !str_starts_with($totals[$colKey] ?? '', 'Rp 0 / Rp 0'))
                    {!! $totals[$colKey] ?? '' !!}
                @endif
            </x-slot>
