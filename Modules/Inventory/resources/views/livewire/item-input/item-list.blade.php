                ...
                <div class="flex items-center transition-all duration-300 ease-out origin-right overflow-hidden"
                    ...
                    @can('inventory.item.create')
                        <div x-data="{ isModalLoading: false }" 
                             ...
                             x-show="!$store.catalog.selectionMode" 
                             x-cloak 
                             class="shrink-0">
                            <button x-on:click="isModalLoading = true; $wire.dispatch('open-item-modal')" 
                                    x-bind:disabled="isModalLoading" 
                                    class="bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-3 py-2 rounded-lg text-sm font-semibold transition-colors flex items-center gap-2">
