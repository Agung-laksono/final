                                ...
                                @if(count($this->searchResults) > 0)
                                    <ul class="max-h-64 overflow-y-auto custom-scrollbar">
                                        @foreach($this->searchResults as $res)
                                            <li @click="addItem({ item_id: {{ $res->id }}, name: {{ json_encode($res->name) }}, code: {{ json_encode($res->code ?? '0001') }}, unit_price: {{ $res->purchase_price ?? 0 }}, image: {{ json_encode($res->image) }}, has_history: {{ \Modules\Purchase\Models\PurchaseOrderItem::where('item_id', $res->id)->whereHas('purchaseOrder', function($q){ $q->where('status', '!=', 'draft'); })->exists() ? 'true' : 'false' }} }); $wire.set('search_query', ''); $wire.show_suggestions = false; window.playSelectSound?.('ting');"
                        ...
                        <div class="relative flex flex-col sm:flex-row bg-emerald-50/30 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800/30 rounded-2xl shadow-sm transition-colors">
                            ...
                            <div class="flex-1 flex flex-col p-4 sm:p-5 relative min-w-0">
                                ...
                                <div class="absolute top-4 right-4 flex items-center gap-2">
                                    ...
                                    <div :class="open ? 'z-50' : ''" x-data="{ open: false, placement: 'bottom', get isRichText() { return item.note && (item.note.includes('<') && item.note.includes('>')); } }">
