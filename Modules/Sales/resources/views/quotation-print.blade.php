    ...
    <div class="flex justify-between items-start mb-8">
        <div>
            @if($quotation->creator->brand && $quotation->creator->brand->logo)
                <img src="{{ Storage::url($quotation->creator->brand->logo) }}" alt="Logo" class="h-20 w-auto object-contain" style="max-height: 80px; height: 80px; width: auto; object-fit: contain;">
            @else
...
<div class="page-container px-8 py-8 relative">
    ...
    <div class="flex justify-between items-start mb-8">
        <div>
            @if($quotation->creator->brand && $quotation->creator->brand->logo)
                <img src="{{ Storage::url($quotation->creator->brand->logo) }}" alt="Logo" class="h-16 w-auto object-contain" style="max-height: 64px; height: 64px; width: auto; object-fit: contain;">
            @else
                <h1 class="text-3xl font-bold text-amber-700 font-serif tracking-tighter">
                    {{ $quotation->creator->brand ? substr($quotation->creator->brand->name, 0, 2) : 'SO' }}
                </h1>
            ...
            @if(!empty($item->custom_attachments) || (isset($item->item->image) && $item->item->image))
                ...
                <div class="absolute inset-0 flex items-center justify-center pointer-events-none select-none z-20 overflow-hidden">
                    <div class="transform -rotate-[30deg] text-white/40 font-black text-2xl tracking-[0.25em] uppercase border-2 border-white/30 px-6 py-2 rounded-lg text-center" style="text-shadow: 0 2px 6px rgba(0,0,0,0.5);">
                        {{ $quotation->creator->brand ? $quotation->creator->brand->name : ($quotation->brand->name ?? 'PERUSAHAAN') }}
                    </div>
                </div>
            @endif
        </div>
