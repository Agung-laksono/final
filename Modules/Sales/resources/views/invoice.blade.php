...
<head>
    ...
    <title>{{ strtolower($order->so_number) }}</title>
...
<div class="page-container px-8 py-8 relative flex flex-col">
    ...
    <div class="flex justify-between items-start mb-8">
        ...
        <div class="text-right">
            <div class="text-2xl mb-1"><span class="text-zinc-900 font-medium">Invoice</span> <span class="font-black">{{ $order->so_number }}</span></div>
            <div class="text-sm text-zinc-900">Tgl: <span class="font-bold">{{ \Carbon\Carbon::parse($order->order_date)->format('d-m-Y') }}</span></div>
            @if($order->deadline)
                <div class="text-sm text-red-600 mt-0.5">Tenggat: <span class="font-bold">{{ \Carbon\Carbon::parse($order->deadline)->format('d-m-Y') }}</span></div>
            @endif
        </div>
    </div>
    ...
    <div class="flex gap-8 mb-8 border-t-2 border-black pt-4">
        ...
        <div class="w-1/2 text-[13px] leading-relaxed">
            ...
            @php 
                $senderPhone = null;
                if($order->brand && $order->brand->phone) { $senderPhone = $order->brand->phone; }
                elseif($order->creator->phone) { $senderPhone = $order->creator->phone; }
            @endphp
            <div class="font-bold text-base uppercase text-black">
                {{ $order->brand ? $order->brand->name : 'PERUSAHAAN' }}
                @if($senderPhone) <span class="font-normal text-[13px] normal-case text-zinc-600 ml-1">({{ $senderPhone }})</span> @endif
            </div>
            @if($order->brand && $order->brand->address)
                <div class="whitespace-pre-line">{{ $order->brand->address }}</div>
            @endif
            
            <div class="mt-1.5 text-zinc-700">
                @php $contacts = []; @endphp
                @if($order->brand && $order->brand->email) @php $contacts[] = $order->brand->email; @endphp @endif
                @if($order->brand && $order->brand->website) @php $contacts[] = $order->brand->website; @endphp @endif
                
                @if(!empty($contacts))
                    <div>{{ implode(' • ', $contacts) }}</div>
                @endif
                
                @if($order->brand && $order->brand->npwp)
                    <div>NPWP: {{ $order->brand->npwp }}</div>
                @endif
            </div>
        </div>
        ...
        <div class="w-1/2 text-[13px] leading-relaxed">
            ...
            <div class="font-bold text-base uppercase text-black">
                {{ $order->customer->name }}
                @if($order->customer->phone) <span class="font-normal text-[13px] normal-case text-zinc-600 ml-1">({{ $order->customer->phone }})</span> @endif
            </div>
            @php
                $customerRegion = collect([
                    $order->customer->province,
                    $order->customer->city,
                    $order->customer->district,
                    $order->customer->village
                ])->filter()->implode(', ');
                $customerDetail = $order->shipping_address ?? $order->customer->address ?? '-';
            @endphp
            @if($customerRegion)
    ...
    <div class="flex justify-between mt-2">
        ...
        <div class="w-1/2">
            @php
                $subtotal = collect($order->items)->sum('subtotal');
                $terbayar = collect($order->payments)->where('status', 'verified')->sum('amount');
                $sisa = max(0, $order->total_amount - $terbayar);
            @endphp
            <table class="totals-table text-sm">
                ...
                <tr>
                    ...
                    <td>{{ number_format($order->shipping_fee + $order->packing_fee, 0, ',', '.') }}</td>
                </tr>
                <tr>
                    <td>Diskon</td>
                ...
                @php
                    $verifiedPayments = collect($order->payments)->where('status', 'verified')->values();
                @endphp
                
                @forelse($verifiedPayments as $idx => $payment)
                <tr>
                    <td>Dp {{ $idx + 1 }}</td>
    ...
    <div class="flex justify-between items-end mt-auto pt-8">
        <div>
            <h3 class="font-bold text-lg mb-1">Cara Pembayaran :</h3>
            @if($order->brand && $order->brand->financeAccounts->isNotEmpty())
                @foreach($order->brand->financeAccounts as $acc)
                    <div class="text-[13px] mb-1">
                        {{ $acc->name }} : {{ $acc->account_number ?: '-' }} a/n {{ $acc->account_holder_name ?: ($order->brand->name ?? '-') }}
                    </div>
                @endforeach
            @else
                <div class="text-sm text-zinc-500">Belum ada pengaturan rekening.<br>Harap hubungi pihak kami.</div>
            @endif
        </div>
        
        <div class="text-center w-56 relative">
            <!-- Simulated Signature (Or replace with img) -->
            <div class="mb-2 relative h-24 flex items-center justify-center">
                @if($order->brand && $order->brand->stamp_image)
                    <img src="{{ Storage::url($order->brand->stamp_image) }}" class="absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 w-24 h-24 object-contain opacity-80 mix-blend-multiply" style="z-index: 10;">
                @endif
                
                @if($order->brand && $order->brand->signature_image)
                    <img src="{{ Storage::url($order->brand->signature_image) }}" class="h-20 object-contain relative mix-blend-multiply" style="z-index: 20;">
                @else
                    <div class="h-20"></div>
                @endif
            </div>
            <div class="font-medium text-black relative" style="z-index: 30;">
                @if($order->brand && $order->brand->director_name)
                    {{ $order->brand->director_name }}
                @elseif($order->brand && $order->brand->financeAccounts->isNotEmpty())
                    {{ $order->brand->financeAccounts->first()->account_holder_name ?: 'Manajemen' }}
                @else
                    {{ $order->creator->name ?? 'Manajemen' }}
                @endif
            </div>
        </div>
    </div>
...
<div class="page-container px-8 py-8 relative">
    <div class="watermark">
        {{ strtoupper($order->brand ? $order->brand->name : 'PERUSAHAAN') }}
    </div>
    ...
    <div class="flex justify-between items-start mb-8">
        ...
        <div class="text-right">
            ...
            <div class="text-sm text-zinc-600">Code: <span class="font-bold text-zinc-900">{{ $order->so_number }}</span></div>
        </div>
    </div>
