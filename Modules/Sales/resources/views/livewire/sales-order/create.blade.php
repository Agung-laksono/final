...
mount(function ($id = null) {
    if ($id) {
        ...
        $prefix = \App\Models\Setting::where('key', 'sales_order_prefix')->value('value') ?: 'ODM-';
        $this->so_number = \App\Services\CodeGenerator::generateNextCode(SalesOrder::class, 'so_number', $prefix);
...
$loadHistory = function () {
    ...
    $raw = \Modules\Sales\Models\SalesOrderItem::with(['salesOrder.customer', 'item.unit'])
        ->join('sales_orders', 'sales_order_items.sales_order_id', '=', 'sales_orders.id')
        ->join('customers', 'sales_orders.customer_id', '=', 'customers.id')
        ->where('sales_order_items.item_id', $this->history_item_id)
        ->where('sales_order_items.unit_price', '>', 0)
        ->where('sales_orders.status', '!=', 'draft')
        ->whereNotNull('sales_orders.so_number')
        ->where('sales_orders.so_number', '!=', '')
    ...
    $this->price_history = array_values(array_filter($raw, function($item) {
        return ($item['unit_price'] ?? 0) > 0
            && !empty($item['sales_order']['so_number'] ?? '')
            && !empty($item['sales_order']['customer']['name'] ?? '');
    }));
};
                                ...
                                @if(count($this->searchResults) > 0)
                                    <ul class="max-h-64 overflow-y-auto custom-scrollbar">
                                        @foreach($this->searchResults as $res)
                                            <li @click="addItem({ item_id: {{ $res->id }}, name: {{ json_encode($res->name) }}, code: {{ json_encode($res->code ?? '0001') }}, unit_price: {{ $res->selling_price ?? 0 }}, image: {{ json_encode($res->image) }}, has_history: {{ \Modules\Sales\Models\SalesOrderItem::where('item_id', $res->id)->whereHas('salesOrder', function($q){ $q->where('status', '!=', 'draft'); })->exists() ? 'true' : 'false' }} }); $wire.set('search_query', ''); $wire.show_suggestions = false; window.playSelectSound?.('ting');"
                                        ...
                                        <div class="relative flex items-center flex-1 sm:w-40 min-w-0 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg" 
                                             ...
                                             x-data="{ 
                                                ...
                                                get validHistory() {
                                                    let history = ($wire.price_history || []).filter(function(h) {
                                                        return h.unit_price > 0 
                                                            && h.sales_order 
                                                            && h.sales_order.so_number 
                                                            && h.sales_order.customer 
                                                            && h.sales_order.customer.name;
                                                    });
                                                    
                                                    if (this.searchQuery.trim() !== '') {
                                                        const q = this.searchQuery.toLowerCase();
                                                        history = history.filter(function(h) {
                                                            const soNumber = (h.sales_order.so_number || '').toLowerCase();
                                                            const customerName = (h.sales_order.customer.name || '').toLowerCase();
                                                            const price = h.unit_price.toString();
                                                            return soNumber.includes(q) || customerName.includes(q) || price.includes(q);
                                                        });
                                                    }
                                                ...
                                                togglePopup(btn) {
                                                    ...
                                                    // Jika ruang di bawah sempit & ruang di atas lebih lega, buka ke atas
                                            ...
                                            />
                                            <button type="button" 
                                                    x-show="item.has_history"
                                                    @click="togglePopup($el)" 
                                                    class="absolute right-2.5 transition-colors text-blue-500 hover:text-blue-600">
                                            ...
                                            <div x-show="open" @click.away="open = false" x-transition 
                                                    ...
                                                    <div class="flex items-center gap-3 flex-1">
                                                        ...
                                                        <div x-show="expanded" x-transition class="flex-1 px-2" x-cloak>
                                                            <div class="relative w-full">
                                                                ...
                                                                <input type="text" x-model="searchQuery" placeholder="Cari nota, customer, harga..." 
                                                ...
                                                <div class="overflow-y-auto p-2 transition-all duration-300" :class="expanded ? 'max-h-[450px]' : 'max-h-64'">
                                                    <template x-for="history in validHistory">
                                                        <div x-data="{ showDetail: false }" x-show="history.unit_price > 0 && history.sales_order?.so_number && history.sales_order?.customer?.name" class="border-b border-zinc-50 dark:border-zinc-800/50 last:border-0 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors group">
                                                            <div @click="item.unit_price = history.unit_price; updateItemSubtotal(index); open = false" 
                                                                 class="p-3 cursor-pointer">
                                                                <div class="flex justify-between items-center">
                                                                    <div class="flex-1">
                                                                        <div class="text-[11px] text-zinc-500 font-medium group-hover:text-cyan-700 dark:group-hover:text-cyan-400 transition-colors" x-text="history.sales_order?.order_date ? (new Date(history.sales_order.order_date)).toLocaleDateString('id-ID', {day: 'numeric', month: 'short', year: 'numeric'}) + ' &bull; ' + (history.sales_order?.so_number || '-') : '-'">
                                                                        </div>
                                                                        <div class="mt-1 flex items-center gap-1.5 flex-wrap">
                                                                            <flux:icon.building-storefront class="w-3.5 h-3.5 text-zinc-400 group-hover:text-cyan-500 transition-colors" />
                                                                            <span class="text-xs font-semibold text-zinc-700 dark:text-zinc-300 group-hover:text-cyan-700 dark:group-hover:text-cyan-400 transition-colors truncate max-w-[120px]" x-text="history.sales_order?.customer?.name || ''" x-show="history.sales_order?.customer?.name"></span>
...
class="text-zinc-400 hover:text-red-600 hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors rounded-full w-8 h-8 flex items-center justify-center p-0" @click="customer = null; $wire.customer_id = ''; $wire.selected_customer = null" tooltip="Ganti Customer"></flux:button>
                ...
                <div x-data="{
                    ...
                    calculateDate() {
                        ...
                        if(this.days === '' || this.days < 0) {
                            ...
                            $wire.deadline = baseDate.toISOString().split('T')[0];
                        }
                        setTimeout(() => { this.isCalculating = false; }, 50);
                    },
                    calculateDays() {
                        this.isCalculating = true;
                        if(!$wire.deadline || !$wire.order_date) {
                            this.days = '';
                        } else {
                            let start = new Date($wire.order_date);
                            let end = new Date($wire.deadline);
                            start.setHours(0,0,0,0);
                            end.setHours(0,0,0,0);
                            let diffDays = Math.round((end - start) / (1000 * 60 * 60 * 24));
                            
                            if (diffDays < 0) {
                                $wire.deadline = $wire.order_date;
                                this.days = 0;
                            } else {
                                this.days = diffDays;
                            }
                    ...
                    <div class="border border-zinc-200 dark:border-zinc-700 rounded-xl p-4 bg-white dark:bg-zinc-900 shadow-sm">
                        ...
                        <flux:input type="date" wire:model="deadline" x-bind:min="$wire.order_date" icon="calendar" class="[&::-webkit-calendar-picker-indicator]:opacity-0 [&::-webkit-calendar-picker-indicator]:absolute [&::-webkit-calendar-picker-indicator]:w-full" />
                    </div>
                </div>
            ...
            <div x-show="step >= 3" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 -translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="sm:col-span-2 md:col-span-1 flex flex-col gap-3">
                <div x-show="grand_total <= 0 || !customer || items.length === 0" x-cloak class="bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 p-3 rounded-xl text-sm border border-red-200 dark:border-red-800 flex items-start gap-2 shadow-sm">
                    ...
                    <div class="flex flex-col">
                        ...
                        <ul class="list-disc pl-4 mt-1 space-y-0.5 text-xs">
                            <li x-show="!customer">Pelanggan belum dipilih.</li>
                ...
                <div class="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex items-center justify-between">
                    <div class="flex gap-2 w-full">
                        ...
                        <flux:button variant="primary" class="w-2/3" @click="submitCart()" x-bind:disabled="isSubmitting || grand_total <= 0 || !customer || items.length === 0">
