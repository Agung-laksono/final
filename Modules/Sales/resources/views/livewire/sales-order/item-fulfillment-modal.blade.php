...
$refreshData = function() {
    ...
    // Only fetch SOs currently in processing (Tarik Barang)
    $activeOrders = SalesOrder::with('items', 'fulfillments')->where('status', 'processing')->get();
    ...
    foreach ($activeOrders as $order) {
        foreach ($order->items as $orderItem) {
            if ($orderItem->item_id == $this->itemId) {
                $neededQty = $orderItem->qty;
                $alreadyConsumed = $order->fulfillments->where('item_id', $this->itemId)->sum('scanned_qty');
...
on(['open-item-fulfillment-modal' => function ($itemId) {
    \Illuminate\Support\Facades\Log::info('Triggered item modal for Sales', ['itemId' => $itemId]);
    $this->itemId = $itemId;
    $this->allocations = [];
    $this->manualBarcode = '';
    $this->refreshData();
...
$autoAllocate = function () {
    ...
    foreach ($this->sos as $so) {
        $toFill = min($so['remaining_needed'], $remainingStock);
        $this->allocations[$so['order_id']]['input_qty'] = $toFill;
...
$handleScan = function ($code, $targetOrderId = null) {
    ...
    if ($targetOrderId) {
        $targetSO = collect($this->sos)->firstWhere('order_id', $targetOrderId);
        if (!$targetSO) return;
        
        $alloc = &$this->allocations[$targetOrderId];
        if ($alloc['input_qty'] >= $targetSO['remaining_needed']) {
            \Flux::toast("Kebutuhan barang untuk {$targetSO['order_number']} sudah terpenuhi.", variant: 'warning');
            return;
        }
        ...
        $alloc['input_qty']++;
        \Flux::toast("Scan berhasil dimasukkan ke {$targetSO['order_number']}.", variant: 'success');
    } else {
        foreach ($this->sos as $so) {
            $alloc = &$this->allocations[$so['order_id']];
            if ($alloc['input_qty'] < $so['remaining_needed']) {
                $alloc['scanned_labels'][] = [
                    'id' => $label->id,
                    'code' => $label->label_code,
                    'warehouse_id' => $label->warehouse_id
                ...
                $alloc['input_qty']++;
                \Flux::toast("Scan dialokasikan otomatis ke {$so['order_number']}.", variant: 'success');
                return;
            }
        }
        \Flux::toast("Seluruh SPK dalam antrean barang ini sudah terpenuhi.", variant: 'warning');
    }
};
...
$save = function() {
    ...
    $totalAllocated = 0;
    foreach ($this->sos as $so) {
        $alloc = $this->allocations[$so['order_id']] ?? null;
        if (!$alloc) continue;
        
        $inputQty = (int) $alloc['input_qty'];
        if ($inputQty < 0) {
            \Flux::toast("Kuantitas input untuk {$so['order_number']} tidak valid.", variant: 'danger');
            return;
        }
        
        if ($inputQty > $so['remaining_needed']) {
            \Flux::toast("Kuantitas input untuk {$so['order_number']} melebihi sisa kekurangan ({$so['remaining_needed']}).", variant: 'danger');
            return;
        }
        
        if ($item->requires_label && count($alloc['scanned_labels']) !== $inputQty) {
            \Flux::toast("Jumlah scan barcode untuk {$so['order_number']} tidak sesuai dengan input kuantitas.", variant: 'danger');
            return;
        }
    ...
    if ($totalAllocated > $this->stock) {
        \Flux::toast("Total alokasi barang ({$totalAllocated}) melebihi stok gudang yang tersedia ({$this->stock}).", variant: 'danger');
        return;
    }
    
    if ($totalAllocated <= 0) {
        \Flux::toast("Harap isi alokasi penyerahan setidaknya pada salah satu SPK.", variant: 'warning');
        return;
    }
    ...
    try {
        DB::transaction(function () use ($item) {
            ...
            foreach ($this->sos as $so) {
                ...
                $order = SalesOrder::find($so['order_id']);
                if (!$order) continue;
                
                if ($item->requires_label) {
                    foreach ($alloc['scanned_labels'] as $sl) {
                        $labelModel = ItemLabel::find($sl['id']);
                        $labelModel->status = 'booked';
                        $labelModel->notes = 'Booked untuk SO: ' . $order->so_number;
                        $labelModel->save();
                        ...
                        $inventoryService->adjustStock(
                            ...
                            'out',
                            $order->so_number,
                            'Fulfillment SO (Batch). Label: ' . $sl['code']
                        );
                    }
                } else {
                ...
                // Cek apakah SO ini sudah lengkap semua item-nya
                $hasDeficit = false;
                $orderWithFulfillments = SalesOrder::with('items', 'fulfillments')->find($order->id);
                foreach ($orderWithFulfillments->items as $oi) {
                    $itemNeeded = $oi->qty;
                    $itemConsumed = $orderWithFulfillments->fulfillments->where('item_id', $oi->item_id)->sum('scanned_qty');
                ...
                if (!$hasDeficit && $order->status === 'processing') {
        ...
        \Flux::toast('Penyerahan barang terpusat berhasil diproses.', variant: 'success');
        ...
        \Illuminate\Support\Facades\Log::error('Fulfillment Terpusat SO Error: ' . $e->getMessage());
...
@php $item = $this->itemId ? Item::with('unit')->find($this->itemId) : null; @endphp
    <div class="p-3 sm:p-5">
        ...
        <div class="mt-4 space-y-2">
            ...
            @forelse($sos as $so)
                @php
                    $alloc = $allocations[$so['order_id']] ?? ['input_qty' => 0, 'scanned_labels' => []];
                    $isDone = $alloc['input_qty'] >= $so['remaining_needed'];
                @endphp
                <div wire:key="so-row-{{ $so['order_id'] }}" class="p-2.5 rounded-xl border border-zinc-200 dark:border-zinc-700 flex flex-col gap-2 {{ $isDone ? 'bg-green-50/20 dark:bg-green-950/10 border-green-200 dark:border-green-900/50' : 'bg-white dark:bg-zinc-800' }}">
                    <div class="flex flex-col gap-2">
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-[10px] font-mono font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 px-1.5 py-0.5 rounded shrink-0">{{ $so['order_number'] }}</span>
                            <div class="text-[10px] text-zinc-500">
                                Butuh: <strong class="text-zinc-700 dark:text-zinc-350">{{ $so['needed'] }}</strong> &bull; Sisa: <strong class="text-zinc-700 dark:text-zinc-350">{{ $so['needed'] - $so['already_consumed'] }}</strong>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between border-t border-zinc-100 dark:border-zinc-700/50 pt-2 text-[11px]">
                            <div class="text-zinc-505">
                                Siap Serahkan: <strong class="text-rose-600 dark:text-rose-450 font-bold">{{ $so['remaining_needed'] }}</strong> <span class="uppercase text-[9px] text-zinc-400">{{ $item?->unit->name ?? 'pcs' }}</span>
                            </div>
                            ...
                            <div class="flex items-center gap-1.5 shrink-0">
                                @if($item?->requires_label)
                                    <div class="flex items-center gap-1">
                                        ...
                                        <flux:button type="button" x-on:click="Flux.modal('camera-scanner-modal').show(); window.dispatchEvent(new CustomEvent('camera-scanner-modal-opened', { detail: { mode: 'continuous' } })); $wire.set('targetOrderId', {{ $so['order_id'] }})" variant="subtle" size="sm" icon="camera" class="!px-2 h-7" title="Scan Khusus SPK Ini" />
                                    </div>
                                @else
                                    <div class="w-20">
                                        @if($stock <= 0)
                                        ...
                                        @else
                                            <flux:input 
                                                type="number" inputmode="numeric" pattern="[0-9]*" wire:model.blur="allocations.{{ $so['order_id'] }}.input_qty" 
                                                min="0" 
                                                max="{{ min($so['remaining_needed'], $stock) }}" 
                            ...
                            <div class="pl-2 border-l-2 border-indigo-200 dark:border-indigo-900/50 flex flex-wrap gap-1 mt-1">
                                @foreach($alloc['scanned_labels'] as $labelIndex => $sl)
                                    <div class="inline-flex items-center gap-1 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300 text-[10px] px-1.5 py-0.5 rounded border border-indigo-100 dark:border-indigo-800/50">
                                        ...
                                        {{ $sl['code'] }}
                                        <button type="button" wire:click="removeScannedLabel({{ $so['order_id'] }}, {{ $labelIndex }})" class="ml-1 text-indigo-400 hover:text-red-500">
            ...
            @empty
                <div class="py-6 border-2 border-dashed border-zinc-200 dark:border-zinc-700 rounded-lg text-center text-xs text-zinc-500">
                    Tidak ada antrean SPK yang membutuhkan barang ini.
                </div>
            @endforelse
        </div>
        ...
        <div class="mt-4 flex justify-end gap-2 pt-3 border-t border-zinc-200 dark:border-zinc-700">
            ...
            <flux:button variant="primary" wire:click="save" wire:target="save" wire:loading.attr="disabled" icon="check" :disabled="count($sos) === 0" class="flex-1 sm:flex-none text-xs">
                Serahkan Semua
            </flux:button>
        </div>
    </div>
